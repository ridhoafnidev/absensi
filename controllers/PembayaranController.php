...
class PembayaranController extends Controller
{
            ...
            */
            ...
            if($request->isGet){
                ...
                return [
                    ...
                    'title'=> "Create new Pembayaran",
                    'content'=>'<span class="text-success">Create Pembayaran success</span>',
                    'footer'=> Html::button('Close',['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
    ...
    public function actionUpdate($id)
    {
        $request = Yii::$app->request;
        $model = $this->findModel($id);    
        $old_file = $model->bukti_pembayaran;   
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                // bukti pembayaran
                $model->bukti_pembayaran = UploadedFile::getInstance($model,'bukti_pembayaran');
                if($model->bukti_pembayaran){
                    $file = $model->bukti_pembayaran->name;
                    if ($model->bukti_pembayaran->saveAs('gambar/pembayaran/'.$file)){
                        $model->bukti_pembayaran = $file;           
                    }
                }
                if (empty($model->bukti_pembayaran)){
                     $model->bukti_pembayaran = $old_file;
                } 
